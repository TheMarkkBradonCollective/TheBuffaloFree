...
Buffalo Buy Nothing ships as a Capacitor Android app. The React web app is bundled into the APK and talks to your production Vercel API for push, map routing, and other server routes.
...
2. Add an Android app with package name `org.buffalobuynothing.app`.
...
```bash
VITE_APP_URL=https://www.buffalobuynothing.com
VITE_SUPABASE_URL=...
VITE_SUPABASE_PUBLISHABLE_KEY=...
VITE_VAPID_PUBLIC_KEY=...
```

`VITE_APP_URL` must be the **www** production origin (`https://www.buffalobuynothing.com`), not the apex host. Apex redirects to www; Capacitor WebView API calls must match that host or neighbors see “Failed to fetch.” You can also set `CAPACITOR_SERVER_URL` to the same www URL.
...
The web app uses `public/Logo.png` (the uploaded Buffalo Free app icon) for favicons, the PWA manifest, and in-app UI. The original upload is also kept as `public/TheBuffaloFree App Logo.png`. The wordmark lockup is `public/TheBuffaloFree.png`. Android launcher icons and splash screens are **native** resources under `android/app/src/main/res/` — they are not picked up automatically from the web bundle.
...
**Neighbors:** share the in-app download page at [www.buffalobuynothing.com/download](https://www.buffalobuynothing.com/download). It compares APK vs home screen installs and shows whether an update is needed.
...
- **Sideload**: Share the signed release APK (`npm run android:apk`) — neighbors use [www.buffalobuynothing.com/download](https://www.buffalobuynothing.com/download).
