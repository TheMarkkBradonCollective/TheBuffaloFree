...
Everything below is pre-filled for **BuffaloBuyNothing** (`org.buffalobuynothing.app`). Your account is already set up — follow these steps in order.

## What is already done

- Release signing keystore: `android/app/buf-buynothing-release.keystore`
...
4. **Contact email** — use your support address (e.g. `support@buffalobuynothing.org` from `.env.example`).
...
(BuffaloBuyNothing is a paid Play download — this tells Google the test account unlocks everything without a separate purchase.)

### Credentials

| Field | Value |
|-------|--------|
| **Email** | `playstore-review@buffalobuynothing.com` |
...
```
BuffaloBuyNothing requires sign-in. Use this test account for full access — no additional purchase needed for review.
...
**Back up** `android/app/buf-buynothing-release.keystore` and `android/keystore.properties` offline. Losing them blocks future updates.
...
| Field | Value |
|-------|-------|
| App name | **BuffaloBuyNothing** |
| Default language | English (United States) |
| App or game | App |
| Free or paid | **Paid** |
| Package name | `org.buffalobuynothing.app` (must match exactly) |
...
```
Buffalo neighbor gifting — give items, request what you need, meet locally.
```

### Full description
```
BuffaloBuyNothing is the community app for Buffalo neighbors who give away items and request what they need — no selling, no bidding, no flipping.

WHAT YOU CAN DO
• Post items to give away or requests for things you need
• Message neighbors safely in-app — no phone numbers required
• Browse by Buffalo neighborhood and coordinate porch pickup
...
This app serves Buffalo-area neighbors only. Pick your neighborhood, connect with people nearby, and keep useful items out of the landfill.
...
BuffaloBuyNothing is run by Markeith White for local, free neighbor-to-neighbor gifting. Items in the community are always free — the app download is a paid Play Store listing.

Privacy policy: https://www.buffalobuynothing.com/privacy
Terms of use: https://www.buffalobuynothing.com/terms
```
...
| Asset | File |
|-------|------|
| Store icon (512×512, site lockup) | `play-store-assets/icon-512.png` (from `public/TheBuffaloFree.png`, not `Logo.png`) |
...
- **Tags:** community, local, gifting, Buffalo

### Contact details
- **Email:** your support email
- **Privacy policy URL:** `https://www.buffalobuynothing.com/privacy`
- **Delete account URL:** `https://www.buffalobuynothing.com/delete-account`
- **Website:** `https://www.buffalobuynothing.com`
...
```
https://play.google.com/apps/testing/org.buffalobuynothing.app
```

Then install from the store listing: `https://play.google.com/store/apps/details?id=org.buffalobuynothing.app`
...
Download (sideload): https://www.buffalobuynothing.com/download
```
...
```
First Google Play release of BuffaloBuyNothing for Buffalo neighbors.
