...
A community gifting PWA for Buffalo neighbors. Post free items, browse a local feed and map, coordinate pickup in chat, RSVP to events, and get push notifications — no money, no ads.

**Live site:** [www.buffalobuynothing.com](https://www.buffalobuynothing.com) · **Android APK:** [www.buffalobuynothing.com/download](https://www.buffalobuynothing.com/download)

**Stack:** React 19, Vite, Tailwind CSS v4, Supabase (auth + Postgres + realtime), Vercel serverless push API.

Branded with Buffalo flag dark blue (`#00338D`) as the primary accent color.
...
npm run android:apk         # signed release for buffalobuynothing.com/download
```
...
Neighbor **Updates** and **News** copy lives in `shared/changelogSeed.ts` and is upserted by cron `/api/cron/publish-changelog` (every 4 hours).
