/**
 * Headless smoke check for The Buffalo Free newspaper skin.
 *
 ...
 *
 *   node scripts/tbf-smoke.mjs [baseUrl]
 */
...
try {
  ...
  // ── Default look is original layout with Buffalo blue accent ──
  ...
  check('default accent is Buffalo blue', defaultAccent.toLowerCase() === '#00338d', defaultAccent);
  ...
  const motion = await page.evaluate(() => ({
    ...
    immersive: document.documentElement.classList.contains('tbf-immersive'),
    calm: document.documentElement.classList.contains('tbf-calm-motion'),
  }));
  check('immersive mode active on desktop', motion.immersive, JSON.stringify(motion));

  check('masthead rendered', (await page.$$('.tbf-masthead')).length > 0);
  const wordmark = await page.evaluate(() => ({
    name: document.querySelector('.tbf-masthead__name')?.textContent.trim() || '',
    free: document.querySelector('.tbf-masthead__free')?.textContent.trim() || '',
  }));
  check(
    'masthead wordmark reads correctly',
    wordmark.name === 'Buffalo' && wordmark.free === 'Free',
    `${wordmark.name} / ${wordmark.free}`,
  );

  const mastheadSize = await page.$eval('.tbf-masthead__name', (el) => parseFloat(getComputedStyle(el).fontSize));
  check('masthead is dramatic (>= 56px on desktop)', mastheadSize >= 56, `${mastheadSize}px`);

  check('paper sits on a desk', (await page.$$('.tbf-desk .tbf-paper')).length > 0);
  check('front page uses lead + rail', (await page.$$('.tbf-front .tbf-front__rail')).length > 0);
  check('section index uses dotted leaders', (await page.$$('.tbf-index__row')).length >= 8);
    ...
    const observer = new MutationObserver((records) => {
      for (const record of records) {
        for (const node of record.addedNodes) {
          if (node.nodeType === 1 && node.classList?.contains('tbf-page-turn')) window.__tsfTurns += 1;
        }
      }
    });
  ...
  check(
    ...
    await calm.evaluate(
      () =>
        document.documentElement.classList.contains('tbf-calm-motion') &&
        !document.documentElement.classList.contains('tbf-immersive'),
    ),
  );
  ...
  await calm.evaluate(() => {
    const raw = localStorage.getItem('tbf_newspaper_experience_v1');
  ...
  check(
    ...
    await calm.evaluate(() => document.documentElement.classList.contains('tbf-immersive')),
  );
  ...
  check('original accent is Buffalo blue', accent.toLowerCase() === '#00338d', accent);
