...
You can keep using the same app you already have. Open it again, or refresh the website, then sign in as usual. If the error is still sitting on the screen, tap Sign out, then sign back in. New APK: https://www.buffalobuynothing.com/download

Sorry for the scare. Thank you for staying with Buffalo Buy Nothing.
...
Download page: https://www.buffalobuynothing.com/download

— Mark

WHERE TO LOOK IN CODE
- src/lib/appOrigin.ts — native WebView uses window.location.origin so /api/* stays on the same host (www).
- src/lib/apkDownload.ts + capacitor.config.ts — canonical origin is https://www.buffalobuynothing.com (VITE_APP_URL / CAPACITOR_SERVER_URL).
...
Install from https://www.buffalobuynothing.com/download. Use the signed release APK (not an old debug file). After install, many website fixes arrive the next time you open the app because the APK loads the live site.
...
BEFORE
localStorage key tbf_saved_items_v1 only — server could not notify when app was closed.
...
Notifications reach your phone when Buffalo Buy Nothing is closed — not only while the tab is open.
...
PostItemModal.tsx category picker; map pin colors in BuffaloMapView.tsx

NEIGHBORHOODS — BUFFALO_NEIGHBORHOODS in src/types.ts`,
...
See who helps run Buffalo Buy Nothing and their role (moderator, administrator, city manager, director).
...
Pick from 38 Buffalo-area neighborhoods when joining or posting — better local matching.

CONST — BUFFALO_NEIGHBORHOODS in src/types.ts
...
MAP — BuffaloMapView.tsx draws polyline overlay
...
DEFAULT TAB — App.tsx initial tab 'map' (localStorage tbf_active_tab_v1)

BuffaloMapView.tsx with category-colored pins.`,
...
UI — BuffaloMapView.tsx color guide toggle / index
...
Install Buffalo Buy Nothing like an app — icon on home screen, standalone display mode, basic offline shell.
...
COMPONENT — BuffaloMapView.tsx (react-leaflet)
...
Pick your neighborhood at onboarding so posts stay local to your part of Buffalo.
...
Warm orange + sage community palette and Buffalo Buy Nothing logo — local feel, not a generic template.

ASSETS — public/Logo.jpeg, CSS variables in src/index.css
siteContent.ts SITE branding copy`,

  '2026-05-19_buffalo-buy-nothing-launches': `WHAT NEIGHBORS SEE
Buffalo Buy Nothing goes live — free local gifting, no selling, no bidding, neighbors helping neighbors.
...
First build session — May 19, 2026 — web app for Buffalo neighbors to give freely and ask kindly.
...
WHAT WE BUILT
${title} — part of the Buffalo Buy Nothing community app at buffalobuynothing.com.
