---
name: runit
description: Full release pipeline for Buffalo Buy Nothing — merge all open PRs first, then sync complete-schema.sql, build website, APK, AAB, commit, deploy, and merge to main. Use when the user says /runit or asks for a full release.
---

# /runit — Full Release Pipeline

Runs the complete Buffalo Buy Nothing release: **merge all open PRs into main first**, then SQL verification, website build, APK + AAB, commit artifacts, push, and print Google Play Console instructions.
...
```
https://www.buffalobuynothing.com/downloads/sac-buy-nothing-beta-v{versionName}.{build}.aab
```

Example: `https://www.buffalobuynothing.com/downloads/sac-buy-nothing-beta-v0.1.0.0029.aab`
...
| Package | `org.buffalobuynothing.app` |
| Upload file | `public/downloads/sac-buy-nothing-beta-v{versionName}.{build}.aab` |
| Public AAB URL | `https://www.buffalobuynothing.com/downloads/sac-buy-nothing-beta-v{versionName}.{build}.aab` |
| Release notes | `play-store-assets/release-notes-v{versionName}-{build}.txt` |
| Path | Testing → Closed testing → Create release |
| Closed testing opt-in | `https://play.google.com/apps/testing/org.buffalobuynothing.app` |
