...
export default function MapNavigationView({
  ...
  useEffect(() => {
    ...
    const tileLayer = L.tileLayer(TBF_MAP_TILE_URL, TBF_MAP_TILE_OPTIONS).addTo(map);
  ...
  const handleShareTrip = async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Buffalo Buy Nothing navigation', text: summary, url: mapsUrl });
        return;
      }
    } catch {
