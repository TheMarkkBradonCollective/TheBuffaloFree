...
export default function PublicNav({ route, onNavigate, hideBrandOnLarge = false }: PublicNavProps) {
  ...
  return (
    <header className={`sticky top-0 z-50 sbn-glass-nav sbn-safe-top ${newspaper ? 'tbf-section-bar' : ''}`}>
      <div
        className={`max-w-5xl mx-auto px-4 ${newspaper ? 'h-[3.75rem]' : 'h-14'} flex items-center gap-3 justify-between min-w-0`}
      >
        <button
          type="button"
          onClick={() => onNavigate('home')}
          aria-label="TheBuffaloFree home"
        ...
        <div className="flex lg:hidden items-center gap-2">
          ...
          <button
            ...
            className={`p-2 rounded-lg border border-app text-app hover:bg-inset transition-colors ${
              newspaper ? 'tbf-section-bar__icon' : ''
            }`}
      ...
      {menuOpen && (
        <nav
          className={`tbf-section-index lg:hidden border-t border-app px-4 py-3 flex flex-col gap-1 ${
