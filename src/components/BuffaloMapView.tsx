...
import { ItemPost, BUFFALO_NEIGHBORHOODS, UserProfile, ITEM_CATEGORIES, ISO_CATEGORIES, extractGPSCoordinates, NEIGHBORHOOD_COORDS, NEIGHBORHOOD_LAT_LONGS, convertPercentToLatLng, CommunityEvent } from '../types';
import {
...
import { TBF_MAP_TILE_OPTIONS, TBF_MAP_TILE_URL } from '../lib/mapTiles';
...
export default function BuffaloMapView({
  ...
  const userNeighborhood = userProfile?.neighborhood || 'Elmwood Village';
  ...
  useEffect(() => {
    ...
    L.tileLayer(TBF_MAP_TILE_URL, TBF_MAP_TILE_OPTIONS).addTo(map);
  ...
  if (isFullScreenMobile) {
    return (
      <div
        id="buffalo_interactive_map_view"
  ...
  return (
    <div id="buffalo_interactive_map_view" className="bg-surface border border-app p-5 rounded-2xl font-sans flex flex-col space-y-4 text-app">
      {navigationOverlay}
      ...
      <div className="relative w-full aspect-square md:aspect-[4/3] bg-inset border border-app rounded-2xl overflow-hidden select-none" id="buffalo_district_grid_canvas_font_sans">
