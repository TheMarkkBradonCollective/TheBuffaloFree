...
export const BUFFALO_NEIGHBORHOODS = [
  'Allentown',
  'Black Rock',
  'Broadway-Fillmore',
  'Central Park',
...
/** Wider Buffalo metro — neighborhood name lookup only, not pickup pin math. */
...
const LEGACY_NEIGHBORHOOD_MAP_COORDS: Record<string, { x: number; y: number }> = {
  'North Buffalo': { x: 52, y: 18 },
  'University Heights': { x: 72, y: 12 },
  'Elmwood Village': { x: 48, y: 28 },
  'Downtown Buffalo': { x: 50, y: 42 },
  'Allentown': { x: 46, y: 36 },
  'West Side': { x: 28, y: 40 },
  'South Buffalo': { x: 58, y: 78 },
  'Lackawanna': { x: 55, y: 88 },
  'Cheektowaga': { x: 82, y: 48 },
  'Williamsville': { x: 90, y: 22 },
};
...
export function findClosestNeighborhood(x: number, y: number): string {
  let closestName: (typeof BUFFALO_NEIGHBORHOODS)[number] = BUFFALO_NEIGHBORHOODS[0];
  let minDistance = Infinity;

  for (const name of BUFFALO_NEIGHBORHOODS) {
