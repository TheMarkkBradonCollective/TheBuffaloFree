...
import {
  ...
  resolveIdentityPhotoUrl,
} from './lib/profilePersistence';

export { isDicebearAvatarUrl };
...
export function normalizeUserProfileRow(row: Record<string, unknown> | null): UserProfile | null {
  ...
  return {
    ...
    neighborhood: String(row.neighborhood ?? 'Buffalo'),
...
export function profileFromAuthUser(user: {
  ...
  return {
    uid: user.id,
    displayName: String(meta.displayName || email.split('@')[0] || 'Buffalo Neighbor'),
    photoURL,
    email: email || 'neighbor@buffalobuynothing.org',
    neighborhood: String(meta.neighborhood || 'Elmwood Village'),
...
async function buildProfileFromLatestComment(uid: string): Promise<UserProfile | null> {
  ...
  return {
    ...
    email: '',
    neighborhood: String(row.userNeighborhood || 'Buffalo'),
    bio: undefined,
    role: 'user',
    createdAt: row.createdAt,
  };
...
export function profileFromListingAuthor(
  ...
  return {
    ...
    email: '',
    neighborhood: listing.neighborhood || 'Buffalo',
    bio: undefined,
    role: 'user',
...
type ProfileUpsertScope = 'full' | 'preferences' | 'bootstrap' | 'identity';
...
async function fetchExistingUserIdentityRow(uid: string): Promise<ExistingUserIdentityRow | null> {
  const { data, error } = await supabase
    .from('users')
    .select('uid, displayName, photoURL, email, createdAt')
    .eq('uid', uid)
    .maybeSingle();

  if (error || !data) return null;
  return data as ExistingUserIdentityRow;
}

function buildUserPrefsPayload(profileForSave: UserProfile) {
  return {
...
function prefsColumnMigrationHint(
  error: { code?: string; message?: string } | null,
  prefsPayload: ReturnType<typeof buildUserPrefsPayload>,
  profileForSave: UserProfile,
): string | null {
  if (!error) return null;
  const errText = `${error.code || ''} ${error.message || ''}`;
  if (!/goGetEnabled|staffInteractionMode|pickupAvailability|goGetRing|navigationSettings|appPreferences|schema cache|PGRST204/i.test(errText)) {
    return null;
  }
...
export async function upsertSupabaseProfile(
  ...
  try {
    ...
    const rawScope = options.scope ?? 'identity';
    const scope: ProfileUpsertScope = rawScope === 'full' ? 'identity' : rawScope;
    ...
    const existing = await fetchExistingUserIdentityRow(profileForSave.uid);
    ...
    if (scope === 'bootstrap') {
      ...
      const displayName = resolveIdentityDisplayName({
        ...
        incomingDisplayName: profileForSave.displayName,
        email,
      });

      const photoURL = photoUrlForProfileUpsert(profileForSave.photoURL);
      const insertPayload = {
        uid: profileForSave.uid,
        displayName,
        ...(photoURL !== undefined ? { photoURL } : {}),
        email,
        neighborhood: profileForSave.neighborhood || 'Buffalo',
        bio: profileForSave.bio?.trim() || null,
        role: profileForSave.role || 'user',
    ...
    const photoURL = resolveIdentityPhotoUrl({
      existingPhotoURL: existing?.photoURL,
      incomingPhotoURL: profileForSave.photoURL,
      allowClearPhoto: options.allowClearPhoto,
    });
    ...
    if (!existing) {
      ...
      if (error && /goGetEnabled|staffInteractionMode|pickupAvailability|goGetRing|navigationSettings|appPreferences|schema cache|PGRST204/i.test(`${error.code || ''} ${error.message || ''}`)) {
        ...
        ({ error } = await supabase.from('users').insert(legacyPayload));
      }
    ...
    if (photoURL) {
      void syncProfilePhotoAcrossApp(profile.uid, photoURL, displayName);
    }
...
const SAVED_ITEMS_STORAGE_KEY = 'tbf_saved_items_v1';
...
function normalizeAppReviewRow(row: Record<string, unknown>): AppReview {
  return {
    ...
    userNeighborhood: String(row.userNeighborhood || 'Buffalo'),
...
export async function upsertSupabaseAppReview(
  ...
  try {
    const payload = {
      ...
      userNeighborhood: author.neighborhood || 'Buffalo',
      rating,
  ...
  try {
    ...
    return (data as Array<{ uid: string; displayName: string; photoURL?: string; neighborhood?: string }>)
      .map((row) => ({
        ...
        neighborhood: row.neighborhood || 'Buffalo',
      }))
  ...
  try {
    ...
    for (const row of recentUsersRes.data ?? []) {
      ...
      activity.push({
        ...
        detail: String(r.neighborhood || 'Buffalo area'),
    ...
    return {
      ...
      activeNeighbors: (activeNeighborsRes.data ?? []).map((row) => {
        ...
        return {
          ...
          neighborhood: String(r.neighborhood ?? 'Buffalo'),
...
export async function staffDeleteUserAccount(params: {
  ...
  try {
    ...
    const targetArea = String(
      (targetProfile as { neighborhood?: string } | null)?.neighborhood || 'Buffalo area',
    );
...
export async function deleteOwnAccount(): Promise<{ ok: boolean; errorMessage?: string }> {
  ...
  const leavingArea = String(
    (leavingProfile as { neighborhood?: string } | null)?.neighborhood || 'Buffalo area',
  );
