...
const STORAGE_KEY = 'tbf_newspaper_experience_v1';
...
export function NewspaperExperienceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    root.classList.toggle('tbf-immersive', prefs.immersiveMode && !motionReduced);
    root.classList.toggle('tbf-calm-motion', motionReduced);
    return () => {
      root.classList.remove('tbf-immersive', 'tbf-calm-motion');
    };
