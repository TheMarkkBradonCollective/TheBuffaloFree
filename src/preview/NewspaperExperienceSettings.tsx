    ...
    <section className="space-y-3 border-t border-app pt-5 mt-5" id="newspaper_experience_settings">
      <div>
        ...
        <p className="text-xs text-muted mt-1 leading-relaxed">
          Sound and motion for The Buffalo Free. Everything here is optional and saved to this device
          {compact ? '. Phones stay quieter by default.' : '.'}
        </p>
      </div>

      <ul className="tbf-experience-list">
        {CONTROLS.map(({ key, label, blurb, icon: Icon }) => {
          ...
          return (
            <li key={key} className="tbf-experience-row">
              <span className="tbf-experience-row__icon" aria-hidden>
                <Icon className="w-4 h-4" />
              </span>
              <div className="min-w-0 flex-1">
                <p className="tbf-experience-row__label" id={`newspaper_experience_${key}_label`}>
                  {label}
                </p>
                <p className="tbf-experience-row__blurb">
