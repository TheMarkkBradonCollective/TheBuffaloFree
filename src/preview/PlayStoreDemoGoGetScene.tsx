...
export default function PlayStoreDemoGoGetScene({ scene }: PlayStoreDemoGoGetSceneProps) {
      ...
      <DemoShell className="p-4">
        <div className="max-w-md mx-auto mt-8 space-y-4">
          <div className="sbn-card p-4">
            ...
            <p className="text-xs text-muted mt-1">Waiting for {DEMO_GOGET_AVERY_PROFILE.displayName} in Elmwood Village</p>
          </div>
  ...
  if (scene === 'goget-navigation') {
    return (
      <MapNavigationView
        origin={BUFFALO_LOCATIONS.downtown}
        destination={{
          lat: BUFFALO_LOCATIONS.elmwood.lat,
          lng: BUFFALO_LOCATIONS.elmwood.lng,
        }}
        destinationLabel={`${BUFFALO_LOCATIONS.elmwood.label} porch`}
        onExit={() => undefined}
        navigationStartMessage={`Go Get pickup for ${chairItem.title}.`}
        navigationFollowUpMessages={['Head to the porch pickup in Elmwood Village.']}
      />
    );
  }
      ...
      <DemoShell className="p-4">
        <div className="max-w-md mx-auto mt-8 space-y-3">
          <div className="sbn-card p-4">
            ...
            <p className="text-xs text-muted mt-1">{BUFFALO_LOCATIONS.elmwood.label}</p>
          </div>
      ...
      <DemoShell className="p-4">
        <div className="max-w-md mx-auto mt-8">
          <div className="sbn-card p-4 space-y-3" id="go_get_arrived_handoff">
            ...
            <p className="text-xs text-muted">{chairItem.title} · {BUFFALO_LOCATIONS.elmwood.label}</p>
