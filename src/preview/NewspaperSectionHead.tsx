...
export default function NewspaperSectionHead({ label, blurb, index }: NewspaperSectionHeadProps) {
  ...
  return (
    <div className="tbf-section-head" aria-hidden="true">
      <span className="tbf-section-head__label">{label}</span>
      {blurb ? <span className="tbf-section-head__blurb">{blurb}</span> : null}
      {index ? <span className="tbf-section-head__index">{index}</span> : null}
    </div>
  );
}
