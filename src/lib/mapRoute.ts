...
export const BUFFALO_SERVICE_AREA = {
  latMin: 42.65,
  latMax: 43.15,
  lngMin: -79.10,
  lngMax: -78.55,
...
export function isLatLngInBuffaloServiceArea(point: LatLng): boolean {
  return (
    Number.isFinite(point.lat) &&
    Number.isFinite(point.lng) &&
    point.lat >= BUFFALO_SERVICE_AREA.latMin &&
    point.lat <= BUFFALO_SERVICE_AREA.latMax &&
    point.lng >= BUFFALO_SERVICE_AREA.lngMin &&
    point.lng <= BUFFALO_SERVICE_AREA.lngMax
  );
}

export function isRouteInBuffaloServiceArea(from: LatLng, to: LatLng): boolean {
  return isLatLngInBuffaloServiceArea(from) && isLatLngInBuffaloServiceArea(to);
}
...
export async function fetchDrivingRoute(
  ...
  try {
    if (!isRouteInBuffaloServiceArea(from, to)) {
