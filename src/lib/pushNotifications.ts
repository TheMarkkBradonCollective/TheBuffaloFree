...
const PUSH_CELEBRATION_DISMISSED_KEY = 'tbf_push_celebration_prompt_dismissed_v1';
const VAPID_CACHE_KEY = 'tbf_vapid_public_key_v1';
...
export function getPushPermissionState(): PushPermissionState {
  if (isNativeApp()) {
    try {
      const stored = localStorage.getItem('tbf_native_push_permission_v1');
...
export async function refreshNativePushPermissionState(): Promise<PushPermissionState> {
  ...
  try {
    localStorage.setItem('tbf_native_push_permission_v1', state);
  } catch {
    // ignore
  }
  return state;
  ...
  if (getPushPermissionState() !== 'granted') {
    return {
      ...
      errorMessage: isNativeApp()
        ? 'Allow notifications for BuffaloBuyNothing in Android settings, then tap Enable alerts again.'
...
export const DIRECTOR_BROADCAST_DEFAULT_TITLE = 'BuffaloBuyNothing';
