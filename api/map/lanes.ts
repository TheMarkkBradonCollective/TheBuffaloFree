...
import { isInBuffaloServiceArea, isValidCoord, parseCoord } from '../_lib/mapCoords';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const point = { lat, lng };
    if (!isInBuffaloServiceArea(point, point)) {
      return res.status(400).json({ error: 'Point must stay within the Buffalo service area' });
    }
