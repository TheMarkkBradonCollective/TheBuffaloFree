...
export async function runDirectorJoinNotify(
  ...
  const neighborhood = String(profile.neighborhood || 'Buffalo area').trim() || 'Buffalo area';

  return runDirectorCategoryAlert(uid, {
    category: 'join',
    title: `New neighbor — ${displayName}`,
    body: `${neighborhood} · joined Buffalo Buy Nothing`,
    excludeUserIds: [uid],
...
export async function runDirectorListingNotify(
  ...
  const neighborhood = String(item.neighborhood || 'Buffalo area');
