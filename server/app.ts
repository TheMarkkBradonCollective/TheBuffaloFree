...
export function createPushApp() {
  ...
  app.get('/api/map/route', async (req, res) => {
    const { parseRouteEndpoints, isInBuffaloServiceArea } = await import('../api/_lib/mapCoords');
    ...
    if (!isInBuffaloServiceArea(parsed.from, parsed.to)) {
      res.status(400).json({ error: 'Route must stay within the Buffalo service area' });
      return;
    }
